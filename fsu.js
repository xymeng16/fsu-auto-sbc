...
(function () {
  ...
  info = {
    ...
    setfield: {
      card: ["pos", "price", "other", "club", "low", "accele"],
      player: [
        "auction",
        "futbin",
        "getprice",
    ...
    formation: {
      27: [0, 5, 5, 5, 12, 10, 10, 16, 25, 18, 25],
  ...
  events.init = async function () {
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (res) {
        ...
        let urlLink =
          "https://mfrasi851i.feishu.cn/wiki/wikcng1Ih7fFRidBfMdNS9SrucR";
        if (res.status == 404) {
        ...
        getAppMain()
          ._FCHeader.getView()
          .__easportsLink.insertAdjacentHTML(
            "afterend",
            `<a class="header_explain" href="${urlLink}" target="_blank">${urlText}</a>`,
          );
      },
      onerror: function () {
    ...
    //获取目标信息
    services.Objectives.objectivesDAO
      .getCategories()
        ...
        if (t.success && t.response && !JSUtils.isString(t.response)) {
          ...
          for (const cat of t.response.categories) {
            for (const group of cat.getGroups()) {
              ...
              if (group.type !== 2) {
                if (
                  group.startTime >= nowDate - 86400 &&
                  group.startTime < nowDate
                ) {
                  tempJson.f = 1;
                  objJson.new["-1"]++;
          ...
          info.task.obj.html = events.taskHtml(
            objJson.new["-1"],
            objJson.reward,
          );
        }
              ...
              switch (v) {
                case 0:
                  return (
                    i.rating >= 0 &&
                    i.rating <= 64 &&
                    (!i.isSpecial() || i.leagueId == 1003 || i.leagueId == 1014)
                  );
                case 1:
                  return (
                    i.rating >= 65 &&
  ...
  events.showLoader = () => {
    document
      .querySelector(".ut-click-shield")
      .classList.add("showing", "fsu-loading");
  ...
  events.hideLoader = () => {
    document
      .querySelector(".ut-click-shield")
      .classList.remove("showing", "fsu-loading");
    document.querySelector(".loaderIcon").style.display = "none";
    if (info.base.template) {
      info.base.template = false;
      isPhone()
        ? cntlr.current()._view._fsuSquad.setInteractionState(1)
        : cntlr.right().getView()._fsuSquad.setInteractionState(1);
    }
  ...
  const fy = function (p) {
    ...
    if (Array.isArray(p)) {
      ...
      t = info.localization.hasOwnProperty(p)
        ? info.localization[p][info.language]
        : p;
    }
    return t;
  };
  ...
  info.localization = {
    ...
    "duplicate.not": [
      "队内不可交易",
      "球會球員無法交易",
      "Club Players are Untradeable",
    ],
    "duplicate.yes": [
      "队内可交易",
      "球會球員可交易",
      "Club Players are Tradable",
    ],
    ...
    "top.readme": [
      "【FSU】插件使用说明",
      "【FSU】插件使用說明",
      "【FSU】Plugin Instructions",
    ],
    "top.upgrade": [
      "有新版FSU插件可升级",
      "有新版本的FSU插件可更新",
      "There is a new version of the FSU plugin that can be upgraded",
    ],
    "notice.upgradefailed": [
      "查询新版本失败",
      "查詢新版本失敗",
      "Query new version failed",
    ],
    "notice.upgradeconfirm": [
      "有新版本点击顶部链接查看",
      "有新版本點擊頂部鏈接查看",
      "There is a new version, click the top link to view",
    ],
    "notice.uasreset": [
      "已重新载入列表 请重新进入查看",
      "已重新載入列表 請重新進入查看",
      "The list has been reloaded, please re-enter to view",
    ],
    "notice.priceloading": [
      "开始读取价格数据 请稍等",
      "開始讀取價格數據 請稍等",
      "Start reading price data, please wait",
    ],
    "notice.loaderror": [
      "读取数据失败 请检查网络",
      "讀取數據失敗 請檢查網絡",
      "Failed to read data, please check the network",
    ],
    "notice.succeeded": [
      "FSU插件加载成功",
      "FSU插件載入成功",
      "FSU plugin loaded successfully",
    ],
    "notice.duplicateloading": [
      "开始读取重复球员数据 请稍等",
      "開始讀取重複球員數據 請稍等",
      "Start reading duplicate player data, please wait",
    ],
    "notice.quicksearch": [
      "使用快捷添加 直接沿用上次配置搜索球员",
      "使用快捷增加球員 直接沿用上次配置搜索球員",
      "Use the shortcut to add, directly follow the last configuration to search for players",
    ],
    "notice.appointloading": [
      "开始读取指定条件球员 请稍等",
      "開始讀取指定條件球員 請稍等",
      "Start reading the specified condition player, please wait a moment",
    ],
    "notice.noduplicate": [
      "已无重复球员",
      "已經沒有重複球員",
    ...
    "notice.quickauction": [
      "球员将按照最低售价作为即买价挂出",
      "球員將按最低價格列在轉會市場上",
      "Players will be listed at the lowest selling price as an immediate purchase price",
    ],
    ...
    "task.nodata": [
      ...
      "沒有數據請過段時間重新進入WEBAPP再查看",
      "No data, please re-enter WEBAPP after a while to check",
    ],
    ...
    "sbc.appoint": [
      "指定条件球员名单",
      "指定條件球員名單",
      "Specified conditions Player list",
    ],
    "sbc.addduplicate": [
      "添加重复球员",
      "新增重複球員",
    ...
    "loadingclose.text": [
      "数据载入 如卡顿点此关闭",
      "數據載入中 如長時間未響應 請點擊此處關閉",
      "If you encounter stuck, click here to close",
    ],
    "quicklist.gotofutbin": [
      "前往FUTBIN查看",
      "前往FUTBIN查看",
      "Go to FUTBIN",
    ],
    "quicklist.auction": [
      "按低价快速拍卖",
      "使用最低價格列入轉會",
      "Quick Auction at Low Price",
    ],
    "pack.promo": ["特殊球员", "特殊球員", "PROMO"],
    "emptylist.t": [
      "处理后无符合条件球员",
      "處理後無符合條件球員",
      "No eligible players after processing",
    ],
    "emptylist.c": [
      "请改变条件或翻页查看",
      "請改變條件或翻頁查看",
      "Please change the criteria or flip the page to view",
    ],
    ...
    "set.player.futbin": [
      "前往FUTBIN查看",
      "前往FUTBIN查看",
      "Go to FUTBIN for Details",
    ],
    "quicklist.getprice": [
      "查询拍卖低价",
      "查詢拍賣低價",
      "Search for Auction Price",
    ],
    "quicklist.getpricey": [
      "刷新拍卖低价",
      "重新整理拍賣低價",
    ...
    "notice.templateload": [
      "读取SBC方案并比价中 请稍后",
      "讀取SBC方案並比價中 請稍後",
      "Reading SBC squad and comparing prices. Please wait.",
    ],
    "notice.templateerror": [
      "阵容保存失败 请重新尝试",
      "陣容儲存失敗 請重新嘗試",
      "Failed to save the squad. Please try again.",
    ],
    "notice.templatesuccess": [
      "阵容填充成功",
      "陣容填充成功",
      "Squad Filled Successfully",
    ],
    "notice.templatezero": [
      "无可加载方案 请稍后再试",
      "無可載入的方案 請稍後再試",
      "Squad failed to save, please try again",
    ],
    ...
    "set.sbc.market": [
      "假想球员拍卖搜索优化",
      "假想球员拍賣搜尋優化",
      "Fantasy Player Auction Search Optimization",
    ],
    "notice.auctionsuccess": [
      "%1 挂牌 %2 成功",
      "%1 掛牌 %2 成功",
      "%1 listed %2 successfully.",
    ],
    "notice.auctionnoplayer": [
      "%1 没有找到球员",
      "%1 沒有找到球員",
      "%1 player not found.",
    ],
    "notice.auctionlimits": [
      "%1 FUTBIN价格超出球员限价",
      "%1 FUTBIN價格超出球員限價",
      "The FUTBIN price for %1 exceeds player limit.",
    ],
    "notice.auctionmax": [
      "已达到拍卖行上限",
      "已達到拍賣行上限",
      "Auction house limit reached.",
    ],
    ...
    "loas.popupt": [
      "球员批量挂拍卖提示",
      "球員批量掛拍賣提示",
      "Bulk Auction Listing Reminder for Players",
    ],
    "loas.popupm": [
    ...
    "loas.variation": [
      "本版块批量拍卖选择球员调整为 %1 个",
      "本版塊批量拍賣選擇球員調整為 %1 個",
      "Batch auction selection of players in this section is adjusted to %1",
    ],
    "loas.start": [
      "程序开始批量售卖球员 预计耗费 %1 秒",
      "程式開始批量售賣球員 預計耗費 %1 秒",
      "The program starts to sell players in bulk, which is expected to take %1 of seconds",
    ],
    "loadingclose.template1": [
      "读取SBC方案列表中 请稍后",
      "讀取SBC方案列表中 請稍後",
      "Read the list of SBC schemes, please wait",
    ],
    "loadingclose.template2": [
      "正在读取方案 %1 阵容 剩余 %2 方案 点此可结束程序",
      "正在讀取和比對方案 %1 陣容 剩餘 %2 方案 點此可結束程式",
      "Reading and comparing plan %1 lineup, remaining %2 plans, click here to end the program",
    ],
    "loadingclose.loas": [
      "正在挂牌第 %1 个球员 剩余 %2 个 点此可结束程序",
      "正在掛牌第 %1 個球員 剩餘 %2 個 點此可結束程式",
      "The %1 players are being listed, and the remaining %2, click here to end the program",
    ],
    "set.player.loas": ["批量拍卖球员", "批量拍賣球員", "Bulk Auction Players"],
    "notice.squaderror": [
      "方案读取失败 可能是FUTBIN无作业方案 请稍后再试",
      "方案讀取失敗 可能是FUTBIN無作業方案 請稍後再試",
      "Scheme reading failed, it may be that FUTBIN has no job scheme, please try again later",
    ],
    "set.getdoc": ["查看设置说明", "檢視設定說明", "View setup instructions"],
    "builder.league": [
      "排除指定联赛球员",
      "排除指定聯賽球員",
      "Exclude designated league",
    ],
    ...
    "notice.packback": [
      "已无未分配球员 自动返回",
      "已無未分配球員 自動返回",
      "No unassigned players, automatically return",
    ],
    "sbc.swapchem": [
      "替换为默契球员",
      "替換為默契球員",
      "Swap Chemistry Players",
    ],
    "notice.notchemplayer": [
      "俱乐部中没有满足当前默契需求的球员",
      "俱樂部中沒有滿足當前默契需求的球員",
      "there are no players in the club who meet the current chemistry needs",
    ],
    "sbc.addchem": ["添加默契球员", "新增默契球員", "Add Chemistry Players"],
    "notice.chemplayerloading": [
      "开始读取满足默契球员 请稍等",
      "開始讀取滿足默契球員 請稍等",
      "Start reading Meet chemistry players, please wait",
    ],
    "sbc.chemplayer": [
      "默契球员名单",
      "默契球員名單",
      "Chemistry Players List",
    ],
    "notice.noplayer": [
      "已无指定条件球员",
      "已無指定條件球員",
      "No conditions specified player",
    ],
    ...
    "notice.nosquad": [
      "已无操作记录 无法法回退",
      "已無操作記錄 無法法回退",
      "There is no operation record and cannot be rolled back",
    ],
    "tile.settitle": ["插件配置", "外掛配置", "Plugin configuration"],
    "tile.settext": [
      "配置FSU功能开关",
      "配置FSU功能開關",
      "Configure FSU function switch",
    ],
    ...
    "tile.fbtext": [
      "有问题请尽快反馈",
      "有問題請儘快反饋",
      "If you have any questions, please give feedback ASAP.",
    ],
    "set.sbc.cback": [
      "假想球员购买自动分配",
      "假想球员购买自动分配",
      "Hypothetical player purchase automatic distribution",
    ],
    "set.sbc.sback": ["阵容回退", "阵容回退", "lineup fallback"],
    "swaptradable.btntext": [
      "批量交换可交易",
      "批量交换可交易",
      "Bulk exchange tradable",
    ],
    "swaptradable.popupt": [
      "批量交换队内可交易球员",
      "批量交换队内可交易球员",
      "Batch exchange of tradable players within the team",
    ],
    "swaptradable.popupm": [
    ...
    "notice.swaptsuccess": [
      "%1 交换成功",
      "%1 交换成功",
      "%1 exchange successful",
    ],
    "notice.swapterror": [
      "%1 交换失败 程序暂停",
      "%1 交换失败 程序暂停",
      "%1 exchange failed, program paused",
    ],
    "loadingclose.swapt": [
      "正在交换第 %1 个球员 剩余 %2 个",
      "正在交换第 %1 个球员 剩余 %2 个",
      "Swap %1 player,%2 remaining",
    ],
    "set.player.swapt": [
      "未分配批量交换可交易",
      "未分配批量交換可交易",
      "Unallocated Bulk Exchange Tradable",
    ],
    "set.sbc.dupfill": [
      "重复球员填充阵容",
      "重複球員填充陣容",
      "Repeat player fill squad",
    ],
    "dupfill.btntext": [
      "重复球员填充阵容",
      "重複球員填充陣容",
    ...
    "autofill.btntext": [
      "一键填充(优先重复)",
      "一鍵填充(優先重複)",
      "One-click fill (priority repeat)",
    ],
    "set.sbc.icount": [
      "搜索球员数量显示",
      "搜尋球員數量顯示",
      "Search number of players displayed",
    ],
    "set.sbc.autofill": [
      "一键填充球员",
      "一鍵填充球員",
      "One-click fill player",
    ],
    "completion.btntext": [
      "一键补全阵容",
      "一鍵補全陣容",
      "One-click complete lineup",
    ],
    "set.sbc.completion": [
      "一键补全阵容",
      "一鍵補全陣容",
    ...
    "notice.seterror": [
      "设置保存失败 请检查",
      "設定儲存失敗 請檢查",
      "Settings failed to save, please check",
    ],
    "shieldlea.btntext": [
      "排除联赛设置",
      "排除聯賽設定",
      "Exclude league settings",
    ],
    "shieldlea.placeholder": [
      "请输入联赛数字ID和英文逗号",
      "請輸入聯賽數字ID和英文逗號",
      "Please enter the league number ID and English comma",
    ],
    "squadcmpl.btntext": [
      "阵容补全(优先重复)",
      "陣容補全(優先重複)",
      "Squad completion (priority repeat)",
    ],
    "squadcmpl.popupt": [
      "阵容补全提示",
      "陣容補全提示",
      "Squad Completion Tips",
    ],
    "squadcmpl.placeholder": [
      "请填入评分和英文逗号组合",
      "請填入評分和英文逗號組合",
      "Please fill in the combination of ratings and English commas",
    ],
    "squadcmpl.placeholder_zero": [
      "无需填充空位",
      "無需填充空位",
      "No need to fill gaps",
    ],
    "squadcmpl.error": [
      "输入填充评分格式不匹配 无法填充指定评分",
      "輸入填充評分格式不匹配 無法填充指定評分",
      "The input fill score format does not match, and the specified score cannot be filled",
    ],
    "set.sbc.squadcmpl": ["阵容补全功能", "陣容補全功能", "Squad completion"],
    "notice.ldatasuccess": [
      "球员数据已全部加载成功",
      "球員資料已全部載入成功",
      "All player data has been loaded successfully",
    ],
    "notice.ldataerror": [
    ...
    "loadingclose.ldata": [
      "正在读取球员数据（%1/%2）请耐心等待",
      "正在讀取球員資料（%1/%2）請耐心等待",
      "Reading player data (%1/%2) please be patient",
    ],
    "uatoclub.btntext": [
      "直接发送%1个至俱乐部",
      "直接傳送%1個至俱樂部",
      "Send %1 directly to the club",
    ],
    "uatoclub.success": [
      "直接发送俱乐部成功",
      "直接傳送俱樂部成功",
      "Send directly to the club successfully",
    ],
    "uatoclub.error": [
      "直接发送俱乐部失败 请进入页面自行分配",
      "直接傳送俱樂部失敗 請進入頁面自行分配",
      "Failed to send the club directly, please enter the page to assign it yourself.",
    ],
    "set.info.skipanimation": [
      "跳过开包动画",
      "跳過開包動畫",
      "Skip the package animation",
    ],
    "builder.untradeable": [
      "仅限不可交易球员",
      "僅限不可交易球員",
    ...
    "set.player.uatoclub": [
      "未分配外部发送至俱乐部",
      "未分配外部傳送至俱樂部",
      "Unassigned external send to club",
    ],
    "douagain.sbctile.title": ["快速SBC", "快速SBC", "Fast SBC"],
    "douagain.packtile.title": ["快速开包", "快速開包", "Quick unpacking"],
    "douagain.sbctile.text": [
      "请先打开或完成SBC",
      "請先開啟或完成SBC",
      "Please open or complete SBC",
    ],
    "douagain.packtile.text": [
      "请先进行开包",
      "請先進行開包",
      "Please open the package first",
    ],
    "douagain.error": [
      "出现程序错误无法打开，请重新完成SBC以便继续。",
      "出現程式錯誤無法開啟，請重新完成SBC以便繼續。",
      "A program error failed to open, please complete the SBC again to continue.",
    ],
    ...
    "set.info.packagain": [
      "商店快速开包",
      "商店快速開包",
      "Store quick open pack",
    ],
    "sbc.infocount": ["已完成 %1 个", "已完成 %1 個", "%1 completed"],
    "notice.dupfilldiff": [
      "请注意因存在于阵容或屏蔽条件未能全部填充球员",
      "請注意因存在於陣容或遮蔽條件未能全部填充球員",
      "Please note that players are not fully filled due to presence in the lineup or shielding conditions",
    ],
    "screenshot.text": [
      "未分配共计 %1 名球员 总价 %2",
      "未分配共計 %1 名球員 總價 %2",
      "Unassigned total %1 players, total price %2",
    ],
    ...
    "tile.gptext": [
      "如有问题可重载入球员",
      "如有問題可重新載入球員",
      "If there is a problem, you can reload the player.",
    ],
    ...
    "set.card.accele": [
      "加速类型（大卡显示）",
      "加速型別（大卡顯示）",
      "AcceleRATE(large card display)",
    ],
    "notice.basesbc": [
      "需要完成初始SBC才可显示更多SBC任务",
      "需要完成初始SBC才可顯示更多SBC任務",
      "The initial SBC needs to be completed to show more SBC tasks",
    ],
    "builder.ignorepos": [
      "忽略球员位置",
      "忽略球員位置",
    ...
    "transfertoclub.popupt": [
      "发送球员提示",
      "傳送球員提示",
      "Send player tips",
    ],
    "transfertoclub.popupm": [
      "是否要将列表中 %1 名球员发送到俱乐部",
      "是否要將列表中 %1 名球員傳送到俱樂部",
      "Do you want to send %1 players in the list to the club",
    ],
    "readauction.error": [
      "读取球员拍卖信息失败，请重试。",
      "讀取球員拍賣資訊失敗，請重試。",
      "Failed to read player auction information, please try again.",
    ],
    "buyplayer.success": [
      "购买球员 %1 成功，花费 %2 。",
      "購買球員 %1 成功，花費 %2 。",
      "Purchase player %1 successfully, cost %2.",
    ],
    "buyplayer.error": [
      "购买球员 %1 失败，%2请稍后再试。",
      "購買球員 %1 失敗，%2請稍後再試。",
      "Purchase of player %1 failed,%2 please try again later.",
    ],
    "buyplayer.error.child1": [
      "被其他用户购买，",
      "被其他使用者購買，",
      "Purchased by other users,",
    ],
    "buyplayer.error.child2": [
      "金币不足，",
      "金幣不足，",
      "Not enough gold coins,",
    ],
    "buyplayer.error.child3": [
      "无拍卖信息，",
      "無拍賣資訊，",
      "No auction information,",
    ],
    "buyplayer.error.child4": [
      "购买超时，",
      "購買超時，",
      "Purchase timed out,",
    ],
    "buyplayer.error.child5": [
      "未分配物品过多，",
      "未分配物品過多，",
      "Too many unallocated items,",
    ],
    "buyplayer.sendclub.success": [
      "购买球员 %1 发送俱乐部成功",
      "購買球員 %1 傳送球隊成功",
      "Buy player %1 send team successfully",
    ],
    "buyplayer.sendclub.error": [
      "购买球员 %1 发送俱乐部失败",
      "購買球員 %1 傳送球隊失敗",
      "Failed to buy player %1 to send team",
    ],
    "readauction.loadingclose": [
      "正在读取最新FUT价格",
      "正在讀取最新FUT價格",
      "Reading the latest FUT prices",
    ],
    "readauction.loadingclose2": [
      "正在读取拍卖信息",
      "正在讀取拍賣資訊",
      "Reading auction information",
    ],
    "buyplayer.loadingclose": [
      "正在尝试购买球员",
      "正在嘗試購買球員",
    ...
    "conceptbuy.btntext": [
      "直接购买此球员",
      "直接購買此球員",
      "Buy this player directly",
    ],
    "set.sbc.conceptbuy": [
      "假想球员直接购买",
      "概念球員直接購買",
      "Concept player direct purchase",
    ],
    "set.player.transfertoclub": [
      "转会发送俱乐部",
      "轉會傳送俱樂部",
      "Transfer sending club",
    ],
    "transfertoclub.unable": [
      "%1个球员因重复无法发送",
      "%1個球員因重複無法傳送",
      "%1 player could not be sent due to duplication",
    ],
    "numberofqueries.btntext": [
      "查询价格次数",
      "查詢價格次數",
    ...
    "numberofqueries.placeholder": [
      "请输入数字 为空重置为5次",
      "請輸入數字 為空重置為5次",
      "Please enter a number, entering empty will reset to 5 times",
    ],
    "settingsbutton.phone": [
      "说明、入口、询价",
      "說明、入口、詢價",
      "desc、entrance、query",
    ],
    "notice.lockplayer": [
      "锁定球员成功",
      "鎖定球員成功",
      "Lock player successfully",
    ],
    "notice.unlockplayer": [
      "解锁球员成功",
      "解鎖球員成功",
      "Unlock Player Success",
    ],
    ...
    "consult.popupt": [
      "请输入导入方案ID或网址",
      "請輸入匯入方案ID或網址",
      "Please enter the import squad ID or URL",
    ],
    "consult.popupm": [
    ...
    "consult.placeholder": [
      "在此填入方案ID或网址",
      "在此填入方案ID或網址",
      "Enter the squad ID or URL here",
    ],
    "consult.error": [
      "未能识别到有效的方案ID或网址，请重新输入。",
      "未能識別到有效的方案ID或網址，請重新輸入。",
      "Could not identify a valid squad ID or URL, please re-enter.",
    ],
    "meetsreq.btntext": [
      "替换为满足需求球员",
      "替換為滿足需求球員",
      "Swap Meets Requirements Players",
    ],
    "set.sbc.meetsreq": [
      "替换满足需求球员",
      "替換滿足需求球員",
    ...
    "meetsreq.error": [
      "俱乐部中没有满足可替换的满足需求球员",
      "俱樂部中沒有滿足可替換的滿足需求球員",
      "There are no replaceable meet requirements players in the club",
    ],
    "set.sbc.templatemode": [
      "SBC方案填充输入模式",
      "SBC方案填充輸入模式",
      "SBC squad populate input mode",
    ],
    "readauction.loadingclose3": [
      "正在读取价格 %1",
      "正在讀取價格 %1",
      "Reading price %1",
    ],
    ...
    "shieldlea.popupm": [
      "默认排除五大联赛，如想调整请从使用说明中找到对应的联赛ID填入。多个ID使用英文逗号分割，否则保存失败。为空恢复为默认设置",
      "預設排除五大聯賽，如想調整請從使用說明中找到對應的聯賽ID填入。多個ID使用英文逗號分割，否則儲存失敗。為空恢復為預設設定",
      "The five major leagues are excluded by default. If you want to adjust it, please find the corresponding league ID from the instructions and fill it in. Multiple IDs are separated by English commas, otherwise the save fails. If it is empty, restore it to the default settings",
    ],
    "popupButtonsText.44401": [
      "前往网站计算",
      "前往網站計算",
      "Go to the website to calculate",
    ],
    "popupButtonsText.44402": [
      "前往查看ID列表",
      "前往檢視ID列表",
      "Go to view ID list",
    ],
    ...
    "sbcrange.concepttitle": [
      "假想搜索无评分范围",
      "概念搜尋無評分範圍",
      "Concept Search No Rating Range",
    ],
    "searchconcept.sameclub": [
      "搜索同俱乐部假想球员",
      "搜尋同俱樂部概念球員",
      "Search concept from the same club",
    ],
    "searchconcept.sameleague": [
      "搜索同联赛同地区假想球员",
      "搜尋同聯賽同地區概念球員",
      "Search concept in the same league and nation",
    ],
    "notice.searchconceptloading": [
      "开始搜索指定条件假想球员",
      "開始搜尋指定條件概念球員",
      "Start searching for specified concept players",
    ],
    ...
    "sbc.onlycmpltext": [
      "保留阵容补全仅为方便查看所需评分",
      "保留陣容補全僅為方便檢視所需評分",
      "Keep the squad complete for convenience only to view the required rating",
    ],
    "set.player.pickbest": [
      "球员挑选最佳提示",
      "球員挑選最佳提示",
      "Player Pick Best Tips",
    ],
    "set.sbc.headentrance": ["顶部SBC入口", "頂部SBC入口", "Top SBC Entrance"],
    "playerignore.popupt": [
      "SBC忽略球员配置",
      "SBC忽略球員配置",
      "SBC ignore player configuration",
    ],
    "playerignore.popupm": [
    ...
    "builder.strictlypcik": [
      "球员挑选严格普通和稀有",
      "球員挑選嚴格普通和稀有",
      "Player Pick SBC Strictly Common and Rare",
    ],
    "headentrance.numberset": [
      "顶部入口数量配置",
      "頂部入口數量配置",
      "Top entrance number",
    ],
    "popupButtonsText.44404": [
      "前往设置排除联赛",
      "前往設定排除聯賽",
      "Go to Settings Exclusion League",
    ],
    "popupButtonsText.44405": [
      "前往设置黄金球员范围",
      "前往設定黃金球員範圍",
      "Go to Set Golden Player Range",
    ],
    "goldenplayer.popupmt": [
      "黄金球员范围设置",
      "黃金球員範圍設定",
      "Golden Player Range Settings",
    ],
    "goldenplayer.popupm": [
    ...
    "goldenplayer.placeholder": [
      "请输入两位数字、最低76、最高99",
      "請輸入兩位數字、最低76、最高99",
      "Please enter two digits, minimum 76, maximum 99",
    ],
    "headentrance.popupmt": [
      "顶部SBC入口数量设置",
      "頂部SBC入口數量設定",
      "Top SBC Entry Quantity Settings",
    ],
    "headentrance.popupm": [
    ...
    "headentrance.placeholder": [
      "请输入1位数字、最低为1、最高为8",
      "請輸入1位數字、最低為1、最高為8",
      "Please enter 1 digit, minimum 1, maximum 8",
    ],
    "sbc.swapgold": [
      "快速替换为同评分黄金",
      "快速替換為同評分黃金",
      "Quickly replace with gold of the same rating",
    ],
  };
  //固话的HTML内容
  html = {
  ...
  UTGameFlowNavigationController.prototype.didPush = function (e) {
    ...
    if (
      info.douagain.hasOwnProperty("SBCListHtml") &&
      info.set.sbc_headentrance
    ) {
      if (
  ...
  UTGameFlowNavigationController.prototype.viewDidAppear = function (...args) {
    ...
    if (
      nav.className == "UTCurrencyNavigationBarView" &&
      info.set.sbc_headentrance
    ) {
  ...
  UTPlayerPicksView.prototype.setItems = function (t, r) {
    ...
    let back = events.createButton(
      ...
      (e) => {
        if (isPhone()) {
          ...
          this._superview._triggerActions(
    ...
    let sPlayers = _.map(t, (i) => {
      return i.isPlayer() && !info.roster.data.hasOwnProperty(i.definitionId)
        ? i.definitionId
        : "";
    }).filter(Boolean);
  ...
  events.playerSelectionSort = (view) => {
    let controller = _.find(view.eventDelegates, {
      className: "UTPlayerPicksViewController",
    });
    if (controller) {
      ...
      let playerArr = _.map(controller.picks, (i, k) => {
        return {
          ...
          f: i.rareflag,
          k: k,
          l: _.includes(leagueOrder, i.leagueId)
            ? _.indexOf(leagueOrder, i.leagueId)
            : 99999,
        };
      });
      let sortKey = ["r", "f", "l"],
      ...
      let bestPlayerElement =
        view.playerPickViews[firstPlayer.k].getRootElement();
  ...
  UTPlayerPicksView.prototype.selectPickOption = function (t, e, i) {
    ...
    if (e.isPlayer() && isPhone() && info.set.player_futbin) {
      ...
      let old =
        this.phoneLargeItem.__optionContainer.getElementsByClassName(
          "fsu-pickspc",
        );
      while (old.length > 0) {
        old[0].parentNode.removeChild(old[0]);
      }
      this.phoneLargeItem.__optionContainer.appendChild(
        this.phoneLargeItem._fsuPlayer.__root,
      );
    }
    ...
    if (info.set.player_pickbest) {
      ...
      if (this.hasOwnProperty("_fsuNoSelected") && this._fsuNoSelected) {
        ...
        _.invokeMap(
          this.getRootElement().querySelectorAll(
            ".player-pick-option.selected",
          ),
          "classList.remove",
          "selected",
        );
  ...
  UTSquadBuilderViewController.prototype.viewDidAppear = function () {
    ...
    if (this.squad.isSBC()) {
      ...
      this._view._searchOptions.__root.appendChild(
        this._view._fsuleague.__root,
      );
  ...
  UTPlayerItemView.prototype.renderItem = function (p, t) {
    ...
    if (p.isValid()) {
      setTimeout(() => {
        ...
        let cr =
          info.set.card_style == 1
            ? `rgb(0,64,166)`
            : `rgb(${ct.dividers.r},${ct.dividers.g},${ct.dividers.b})`;
        ...
        //价格区块
        let pp = info.roster.data.hasOwnProperty(p.definitionId)
          ? info.roster.data[p.definitionId].prices[info.base.platform].LCPrice
          : "0";
        ...
        if (p.lastSalePrice !== 0) {
          ...
          ptl = Number(pp)
            ? events.priceLastDiff(pp.replace(/,/g, ""), p.lastSalePrice)
            : "<span></span>";
        }
        ...
        let plow = info.base.price.hasOwnProperty(p.rating)
          ? `<div class="fsu-other-low currency-coins">${fy("duplicate.lowprice")}${Number(info.base.price[p.rating]["p"]).toLocaleString()}</div>`
          : `<span class="fsu-other-low"></span>`;

        let pOtherPos = otherPos.length
          ? `<div class="fsu-other-pos">${otherPos.join(" / ")}</div>`
          : `<span class="fsu-other-pos"></span>`;
        ...
        if (p.duplicateId) {
          if (
            services.Item.itemDao.itemRepo.club.items._collection.hasOwnProperty(
              p.duplicateId,
            )
          ) {
            pe = services.Item.itemDao.itemRepo.club.items._collection[
              p.duplicateId
            ].untradeable
              ? 0
              : 1;
        ...
        if (!p.isGK() && info.set.card_accele && p.getMetaData()) {
          ...
          fcAccele.innerHTML = fy(
            `player.accelerate${events.countPlayerAccele(p.getMetaData().height, p.getMetaData().attributes[2].rating, p.getMetaData().attributes[0].rating, p.getMetaData().attributes[6].rating)}`,
          );
        ...
        if (this.__root.classList.contains("small")) {
          ...
          for (let i in pm) {
            if (document.querySelector(pm[i])) {
              if (
                !document
                  .querySelector(pm[i])
                  .parentNode.querySelectorAll(".fsu-cards").length
              ) {
                cs = Number(i);
                break;
              }
          ...
          if (cs == 7 && document.querySelector(".icon-transfer.selected"))
            cs = 4;
          if (cs == 8 && document.querySelector(".icon-club.selected")) cs = 9;
          if (cs == 2 && controller.className == "UTWatchListViewController")
            cs = 11;
          if (
            cs == 8 &&
            controller.className == "UTAcademyPlayerFromClubViewController"
          )
            cs = 3;
          //console.log(cs)
          if (cs !== 0) {
            ...
            if ([5, 21, 22, 23, 24, 25, 31].includes(cs)) {
              ...
              if ([1, 2, 4, 7, 11].includes(cs)) {
                pbc = "top";
                if (isPhone()) {
                  this.getRootElement().parentNode.querySelector(
                    ".name",
                  ).style.width = "25%";
                }
              }
              ...
              if (cs == 4 || cs == 7 || cs == 11) {
                this._fsuCardPriceBox
                  .querySelector(".fsu-price-last")
                  ?.remove();
            ...
            if ([8, 9].includes(cs) && playerLock) {
              this.getRootElement()
                .parentNode.querySelector(".name")
                .classList.add("fsulocked");
            }

            //阵容刷新后购买失败标识添加
            if (
              controller.className.includes("UTSBCSquad") &&
              cs == 21 &&
              "_fsuBuyEroor" in controller._squad &&
              controller._squad._fsuBuyEroor.includes(pId) &&
              p.concept
            ) {
          ...
          if (
            ...
            this._fsuLock = events.createButton(
              ...
              (e) => {
                ...
                e.getRootElement().classList.add(
                  playerLock ? "unlock" : "lock",
                );
                if (!isPhone()) {
            ...
            if (
              (classNameRegex.test(controller.className) ||
                (isPhone() &&
                  controller.className == `ItemDetailsViewController`)) &&
              document.querySelector(`.slider span[p-id="${p.id}"]`) &&
              !cardParen.classList.contains("reward")
            ) {
              cardParen.insertBefore(
                this._fsuLock.getRootElement(),
                cardParen.firstChild,
              );
            }
          }
          ...
          //大卡预览处增加购买失败描述
          if (
            "_squad" in controller &&
            "_fsuBuyEroor" in controller._squad &&
            controller._squad._fsuBuyEroor.includes(pId) &&
            p.concept &&
            cardParen.classList.contains("tns-item")
          ) {
          ...
          //战术编辑处调整大卡片的属性显示错误。
          if (
            (cardParen.classList.contains(
              "ut-tactics-instruction-menu-view--item-container",
            ) ||
              cardParen.classList.contains("main-reward")) &&
            cardParen.classList.length === 1
          ) {
            cardParen.style.position = "relative";
          }
        }
  ...
  UTSquadOverviewViewController.prototype.viewDidAppear = function () {
    ...
    let p = this._squad._players
      .map(function (i) {
        if (
          i._item.type == "player" &&
          !info.roster.data.hasOwnProperty(i._item.definitionId)
        ) {
    ...
    if (this._squad.isSBC()) {
      ...
      for (let i = 1; i < 11; i++) {
        ...
        if (
          events.getDedupPlayers(
            events.getItemBy(1, { rating: y - i }),
      ...
      for (let i of yl) {
        ...
        sp._fsuRlist[n] = r;
        sp._fsuQuickRight
          .querySelector(".fsu-quick-list")
          .append(sp._fsuRlist[n].__root);
      }
      ...
      sp._fsuQuickRight
        .querySelector(".fsu-quick-list")
        .append(quickUnassignedBtn.getRootElement());

      //开始判断是否需要屏蔽重复按钮
      let unassignedIds = _.uniq(
        _.map(repositories.Item.getUnassignedItems(), `definitionId`),
      );
      if (unassignedIds.length) {
        if (
          events.getDedupPlayers(
            events.getItemBy(2, { definitionId: unassignedIds }),
            this._squad.getPlayers(),
          ).length
        ) {
          quickUnassignedBtn.setInteractionState(1);
      ...
      sp._fsuQuickRight
        .querySelector(".fsu-quick-list")
        .append(quickTransfersBtn.getRootElement());
      ...
      if (transferIds.length) {
        if (
          events.getDedupPlayers(
            events.getItemBy(2, { definitionId: transferIds }),
            this._squad.getPlayers(),
          ).length
        ) {
          quickTransfersBtn.setInteractionState(1);
      ...
      if (info.set.sbc_sback) {
        ...
        sp._fsuQuickRight
          .querySelector(".fsu-quick-list")
          .append(sp._fsuRlist["r_b"].__root);
      }
  ...
  UTSectionedItemListView.prototype.addItems = function (t, e, i, r) {
    ...
    let p = this.listRows
      .map(function (i) {
        if (
          i.data.type == "player" &&
          !info.roster.data.hasOwnProperty(i.data.definitionId)
        ) {
    ...
    if (
      ...
      cntlr.current().getNavigationTitle() !==
        services.Localization.localize("navbar.label.watchlist") &&
      (cntlr.current().getNavigationTitle() !==
      ...
      for (let n of this.listRows) {
        if (
          ...
          n.data.loans == -1 &&
          n.data.type == "player" &&
          !n.data._auction.isClosedTrade() &&
          !n.data._auction.isActiveTrade()
        ) {
          pn++;
          ...
          n._fsuLosAuction = events.createToggle(
            ...
            async (e) => {
              ...
              e._parent._fsuAkbToggle.toggle(
                e._parent._fsuAkbCurrent == e._parent._fsuAkbNumber,
              );
        ...
        this._fsuAkbToggle = events.createToggle(
          ...
          async (e) => {
            ...
            for (let n of e._parent.listRows) {
              if (
                n.hasOwnProperty("_fsuLosAuction") &&
                n._fsuLosAuction._interactionState
              ) {
                n._fsuLosAuction.toggle(sf);
                if (sf) {
                  if (n.hasOwnProperty("_fsuLosAuction")) {
                    if (events.getCachePrice(n._fsuLosAuction._pId, 2)) {
                      e._parent._fsuAkbArray[n._fsuLosAuction._id] =
                        n._fsuLosAuction;
                    }
                  }
                }
        ...
        this._fsuAkbButton = events.createButton(
          ...
          (e) => {
            events.popup(
              ...
              fy([
                "loas.popupm",
                e._parent._fsuAkb.querySelector(".fsu-akb-num").innerText,
                e._parent._fsuAkb.querySelector(".fsu-akb-price").innerText,
              ]),
              (t) => {
                if (t === 2) {
      ...
      if (z == 44404) {
        events.popup(
          ...
          (t, i) => {
            ...
            if (t === 44402) {
              GM_openInTab(
                `https://mfrasi851i.feishu.cn/wiki/Hx8KwfYznimX8KkpK0icaEm4nYc#UbN2dpki5o23C9xKuRBcETK3nhg`,
  ...
  events.getCachePrice = (i, t) => {
    if (t == 1) {
      ...
      if (i in info.roster.data) {
        ...
        price = isNaN(untreated)
          ? Number(untreated.replace(/,/g, ""))
          : Number(untreated);
      }
      return price;
    } else {
      return (
        info.roster.data.hasOwnProperty(i) && events.getCachePrice(i, 1) !== 0
      );
    }
  };
  events.losAuctionCount = (e, t) => {
    ...
    if (document.querySelector(".fsu-screenshot")) {
      ...
      view._view.__root.querySelector(
        ".fsu-screenshot h2 span.text",
      ).textContent = fy([
        "screenshot.text",
        price.length,
        price.reduce((a, b) => a + b, 0).toLocaleString(),
      ]);
    }
  };
  ...
  UTPaginatedItemListView.prototype.renderItems = function (t) {
    ...
    if (
      !isPhone() &&
      c.hasOwnProperty("_rightController") &&
      c._rightController
    ) {
      c = cntlr.right()._parentViewController;
    }
    if (csbc) {
      if (
        c.getNavigationTitle() ==
        services.Localization.localize("navbar.label.clubsearch")
      ) {
        let s = [];
        if (
          c._fsuFillArray.length &&
          c._currentController.searchCriteria.defId.length &&
          this.listRows.length
        ) {
          s = this.listRows
            .map((i) => {
              if (
                c._currentController.searchCriteria.defId.includes(
                  i.data.definitionId,
                )
              ) {
                return i.data.definitionId;
            ...
            .filter(Boolean);
          if (!s.length) {
            this.__itemList.prepend(
              events.createDF(
                `<div class="ut-no-results-view"><div class="contents"><span class="no-results-icon"></span><h2>${fy("emptylist.t")}</h2><p>${fy("emptylist.c")}</p></div></div>`,
              ),
            );
          } else {
        ...
        if (
          info.set.sbc_market &&
          pn.hasOwnProperty("pinnedItemView") &&
          pn.pinnedItemView &&
          pn.pinnedItemView.itemCell.data.concept
        ) {
          let z = 0;
  ...
  UTStoreRevealModalListView.prototype.addItems = function (e, t, i, o) {
    ...
    let p = e
      .map(function (it) {
        if (
          it.type == "player" &&
          !info.roster.data.hasOwnProperty(it.definitionId)
        ) {
  ...
  UTSquadEntity.prototype.getRating = function () {
    ...
    events.squadTotal(
      this.getFieldPlayers()
        .map(function (i) {
          if (info.roster.data.hasOwnProperty(i._item.definitionId)) {
            return i._item.definitionId;
          }
        })
        .filter((i) => i > 0),
  ...
  events.squadTotal = (p) => {
    ...
    if (e) {
      ...
      if (n !== 0 && document.getElementById("sbcPrice")) {
        ...
        p.append(
          events.createDF(
            events.priceLastDiff(
              n,
              document.getElementById("sbcPrice").innerText.replace(/,/g, ""),
            ),
          ),
        );
      }
  ...
  events.loadPlayerPrice = async (list, el) => {
    if (list.length > 0) {
      ...
      for (let i = 0; i < gr; i++) {
        ...
        pu.push(
          `https://www.futbin.com/${info.base.year}/playerPrices?player=&rids=${lt}`,
        );
      }
      for (let k in pu) {
          ...
          if (e.length > 0) {
            for (let i of e) {
              ...
              if (i.querySelectorAll(lastPriceName).length > 0) {
                i.querySelector(".fsu-price-val .title span").outerHTML =
                  events.priceLastDiff(
                    p.replace(/,/g, ""),
                    isPhone()
                      ? i
                          .querySelector(lastPriceName)
  ...
  UTBaseSquadSplitViewController.prototype.viewDidAppear = function () {
    ...
    if (info.set.info_squad) {
      ...
      this._getLeftController()
        .getView()
        .getSummaryPanel()
        .getRootElement()
        .querySelector(
          `.ut-squad-summary-info${this.className == "UTSBCSquadSplitViewController" ? "--right" : ""}`,
        )
        .after(
          events.createDF(
            `<div class="fsu-squad-pBox"><div class="fsu-squad-pWrap"><span class="fsu-squad-pTitle">${fy("sbc.topsquad")}</span><span id="squadTotal" class="fsu-squad-pValue currency-coins">0</span></div></div>`,
          ),
        );
      ...
      if (this._squad.isSBC()) {
        ...
        if (info.task.sbc.stat.hasOwnProperty(this._set.id)) {
          if (this._set.challengesCount == 1) {
            si = info.task.sbc.stat.hasOwnProperty(this._set.id)
              ? info.task.sbc.stat[this._set.id]
              : -1;
          } else {
            if (info.task.sbc.stat[712].hasOwnProperty("c")) {
            }
            si = info.task.sbc.stat[this._set.id].hasOwnProperty("c")
              ? info.task.sbc.stat[this._set.id].c[this._challengeId]
              : -1;
          }
        } else {
          si = -1;
        ...
        if (si !== -1 && !document.getElementById("sbcPrice")) {
          this._getLeftController()
            ...
            .after(
              events.createDF(
                `<div class="fsu-squad-pWrap"><span class="fsu-squad-pTitle">${fy("sbc.topprice")}</span><span id="sbcPrice" class="fsu-squad-pValue currency-coins">${Number(info.base.platform == "pc" ? si.pc : si.tv).toLocaleString()}</span></div>`,
              ),
            );
        }
      }
  ...
  events.squadCount = (e) => {
    ...
    let dli = [
      ...
      cs = Array.from({ length: 30 }, (_, i) => i + br).filter(
        (n) => !dli.includes(n),
      ),
      l = cs.length ? `&lock=${cs.join(",")}` : "";
    GM_openInTab(`https://futcd.com/sbc.html?target=${t}${pr}${l}`, {
      active: true,
      insert: true,
      setParent: true,
    });
  ...
  events.squadConsult = (e) => {
    ...
    GM_openInTab(
      `https://www.futbin.com/squad-building-challenges/ALL/${i}/list`,
      { active: true, insert: true, setParent: true },
    );
  };
  ...
  events.SBCSetRatingPlayers = async (e) => {
    ...
    if (phone) {
      if (cntlr.current().className == "UTSBCSquadDetailPanelViewController") {
        ...
        cntlr.current().className == "UTSBCSquadOverviewViewController"
      ) {
    ...
    if (
      e.getRootElement().tagName == "BUTTON" &&
      e.getRootElement().classList.length < 3
    ) {
      needFind = false;
    }
    ...
    switch (queryType) {
      case "d":
        ...
        break;
      case "t":
        pendingPlayers = events.getItemBy(2, {
          definitionId: _.uniq(
            _.map(repositories.Item.getTransferItems(), (i) => {
              if (i.getAuctionData().isInactive()) {
                return i.definitionId;
    ...
    if (queryType !== "conceptsearch") {
      resultPlayers = events.getDedupPlayers(
        pendingPlayers,
        currentSquad.getPlayers(),
      );
    ...
    //点击选中位置
    if (needFind) {
      let slotIndex = _.find(
        currentSquad.getNonBrickSlots(),
        (item) => !item.isValid() && !item.isBrick(),
      )?.index;
      if (slotIndex) {
    ...
    let detailsController = phone
      ? cntlr.current()._rootController
      : cntlr.right();
    if (queryType == "d") {
      ...
      events.sbcQuerySetFillAttr(
        detailsController._parentViewController,
        queryType == "conceptsearch" ? 9 : 5,
        resultPlayers,
        querySort,
      );
  ...
  events.sbcQuerySetFillAttr = (element, type, players, sort) => {
    ...
    element._fsuFillArray =
      players.length || _.isObject(players) ? players : [];
    element._fsuFillRange = players.length
      ? [_.minBy(players, "rating").rating, _.maxBy(players, "rating").rating]
      : [46, 99];
    if (sort !== false) {
    ...
    setTimeout(() => {
      if (info.set.info_sbc) {
        for (let i = 0; i < e.length; i++) {
          let data = e[i],
            tap = this._SBCCategoriesTM.items[i],
            count = 0,
            expiry = 0;
          ...
          if (count) {
            tap.__root.append(
              events.createDF(
                fy(html.taskBar).replace("{Number}", `+${count}`),
              ),
            );
          }
          if (data.isFavourite && expiry) {
            tap.__root.append(
              events.createDF(
                `<div class="fsu-task-bar-favorite">${expiry}</div>`,
              ),
            );
          }
        }
    ...
    if (info.set.info_sbc) {
      ...
      if (t) {
        if (t.isFavourite) {
          ...
          if (se > 0) {
            el.append(
              events.createDF(`<div class="fsu-task-bar-favorite">${se}</div>`),
            );
          }
        }
      } else {
  ...
  events.sbcFilter = (e) => {
    let t = cntlr.current()._view.sbcSetTiles,
      g = e.getIndex();
    for (let i of t) {
      let y = true,
        d = i.data.id;
      ...
      if (_.isArray(this.sbset.awards)) {
        _.map(this.sbset.awards, (item, index) => {
          if (item.isItem && item.item.isPlayer()) {
            ...
            btn
              .getRootElement()
              .setAttribute("data-name", `${item.item.getStaticData().name}`);
            this.getView()
              ._setInfo._rewards.__rewardList.querySelector(
  ...
  events.openFutbinPlayerUrl = async (e) => {
    ...
    events.hideLoader();
    GM_openInTab(`https://www.futbin.com/${info.base.year}/player/${fi}/`, {
      active: true,
      insert: true,
      setParent: true,
    });
  ...
  events.sbcInfoFill = (d, e) => {
    ...
    if (s !== undefined) {
      if (e.hasOwnProperty("__tileTitle") && s.f == "1") {
        ...
        e.getRootElement().insertBefore(
          events.createDF(`<div class='fsu-objnew'>${fy("task.new")}</div>`),
          e.getRootElement().firstChild,
        );
      }
      if (
        !e.__root.querySelector(".task-expire") &&
        "data" in e &&
        !e.data.isComplete()
      ) {
        ...
        e.__root.prepend(
          events.createDF(
            `${expireTime < 86400 && !e.data.notExpirable ? "<div class='task-expire'>" + fy("task.expire") + "</div>" : ""}`,
          ),
        );
      }
    }
      ...
      if (e.data.awards[0].isPack) {
        ...
        if (packCoinValue) {
          ...
          let packTitle = events.createElementWithConfig("div", {
            textContent: _.replace(
              _.replace(fy("packcoin.text"), ":", ""),
              "：",
              "",
            ),
          });
          ...
          e.__mainReward
            .querySelector(".ut-pack-graphic-view")
            .appendChild(packBox);
        }
      }
    }
        ...
        for (let i of e) {
          if ("_fsuSubSet" in i) {
            let sId = i._fsuSubSet.id,
              ...
              priceValue = Number(
                info.base.platform == "pc"
                  ? info.task.sbc.stat[id].c[sId].pc
                  : info.task.sbc.stat[id].c[sId].tv,
              ).toLocaleString(),
  ...
  UTMarketSearchFiltersView.prototype.setPinnedItem = function (e, t) {
    ...
    let sbc = isPhone()
      ? cntlr.current().squadContext.squad.isSBC()
      : cntlr.current()._squad.isSBC();
  ...
  events.detailsButtonSet = (e) => {
    ...
    if (e.item.isPlayer()) {
      ...
      if (
        ...
        e.item.concept &&
        "_fsuConceptBuy" in a &&
        events.getCachePrice(pid) &&
        info.set.sbc_conceptbuy
      ) {
      ...
      if (
        ...
        w.getParentViewController().className ==
        ...
        e.item.definitionId ==
          w
            ...
            ._squadContext.squad.getPlayer(
              w.getParentViewController()._squadContext.slotIndex,
            ).item.definitionId &&
        w
          .getParentViewController()
          ._squadContext.squad.getPlayer(
      ...
      if (pid && a.hasOwnProperty("_fsuPlayer")) {
        ...
        a._fsuPlayer.__root.setAttribute(
          "data-name",
          `${e.item._staticData.name}`,
        );
      ...
      if (
        a.hasOwnProperty("_fsuQuickGlod") &&
        e.item.rating > 75 &&
        e.item &&
        e.item.isSpecial() &&
        e.item.leagueId !== 1003 &&
        e.item.leagueId !== 1014
      ) {
        let defId = _.map(w.squad.getPlayers(), "item.definitionId");
        let tempC = { rating: e.item.rating, rs: 9 };
        tempC.NEdatabaseId = defId;
        let goldList = events.getItemBy(
          2,
          events.ignorePlayerToCriteria(tempC),
        );
        if (goldList.length) {
          let playerIndex = _.findIndex(
            w.squad.getPlayers(),
            (i) => i.item.definitionId == e.item.definitionId,
          );
    ...
    if (a.hasOwnProperty("_fsuSwap") && a.hasOwnProperty("_fsuUn")) {
      ...
      let ul = cntlr
        ...
        .filter(Boolean);
      let rul = services.Item.itemDao.itemRepo
        .getUnassignedItems()
        .map((i) => {
          if (i.isDuplicate() && !i.isLoaned() && i.isPlayer()) {
            if (ul.length) {
      ...
      //SBC状态置为0
      if (
        w.hasOwnProperty("_parentViewController") &&
        w._parentViewController
      ) {
      ...
      //默契球员按钮判断
      if (
        w.hasOwnProperty("challenge") &&
        w.viewmodel.getIndex() < 11 &&
        w.squad.getFieldPlayers().filter((i) => i.getItem().rating > 0).length
      ) {
        let c = 0,
          r = 0,
          q = 0;
      ...
      if (
        w.hasOwnProperty("challenge") &&
        w.challenge.meetsRequirements() &&
        info.set.sbc_meetsreq &&
        w.viewmodel.getIndex() < 11
      ) {
        a._fsuMeets.show();
    ...
    if (
      ...
      a._fsuConceptSearch = [];
      let btnSetting = {
        club: [`teamId:club`, `leagueId:league`],
        league: [`leagueId:league`, `nationId:nation`],
      };
    ...
    if (
      ...
      _.map(w.squad._fsuRequests, (i) => {
        ...
        btn.setSubtext(
          `${w.challenge.getRequirementCounter(i.value)}/${i.value.count}`,
        );
    ...
    function combine(t) {
      return _.map(t, function (value, index, array) {
        return index < array.length - 1
          ? value + " " + _.toUpper(L10n.localize("label.general.or"))
          : value;
      }).join(" ");
    }
    switch (rKey) {
    ...
    if (e.hasOwnProperty("_btnAddSwap") && cntlr.current()._squad.isSBC()) {
      ...
      let fu = events.createButton(
        ...
        async () => {
          ...
          let p = events.getDedupPlayers(
            events.getItemBy(2, {
              definitionId: services.Item.itemDao.itemRepo
                .getUnassignedItems()
                .map((i) => {
                  if (i.isDuplicate() && !i.isLoaned() && i.isPlayer()) {
                    return i.definitionId;
      ...
      let qsg = events.createButton(
        ...
        (e) => {
          console.log(e);
          events.showLoader();
          let playerIndex = e.challenge.squad
            ._getPlayerSlotByItemId(e.oldPlayer.id)
            .getIndex();
          let playerList = _.cloneDeep(
            _.map(e.challenge.squad.getPlayers(), (i) => i.item),
          );
  ...
  events.SBCSetChemPlayers = async (e) => {
    ...
    for (let player of shortlist) {
      currentList[playerIndex] = player;
      let chemResult = squad.chemCalculator.calculate(
        formation,
        currentList,
        manager,
      );
      ...
      let playerChemNeed = isPlayer
        ? changePlayerChem >= playerChemAdjust
        : changePlayerChem > playerChemAdjust;
      if (
        countChem >= needChem ||
        (countChem >= currentChem && playerChemNeed)
      ) {
        resultList.push(player);
      }
    }
  ...
  UTObjectivesHubView.prototype.setupNavigation = function (e) {
    ...
    for (let i of t) {
      if (info.task.obj.stat.new.hasOwnProperty(i.id)) {
        ...
        i.__root.append(
          events.createDF(
            fy(html.taskBar).replace(
              "{Number}",
              `+${info.task.obj.stat.new[i.id]}`,
            ),
          ),
        );
      }
  ...
  UTObjectiveCategoryView.prototype.setCategoryGroups = function (i, e, o, n) {
    ...
    for (let i of g) {
      ...
      if (info.task.obj.stat[i.id].e) {
        i.__title.parentNode.after(
          events.createDF(
            `<div class="task-expire">${fy("task.expire")}</div>`,
          ),
        );
      }
      ...
      if (
        item &&
        item.rewards.rewards.length &&
        item.rewards.rewards.length == 1
      ) {
        let reward = item.rewards.rewards[0];
        let rewardText = UTLocalizationUtil.buildRewardString(
          reward,
          services.Localization,
        );
  ...
  UTHomeHubView.prototype._generate = function (...args) {
    if (!this._generated) {
      ...
      let locale =
        services.Messages.messagesDAO.authDelegate.sessionUtas.locale;
  ...
  events.reloadPlayers = async (e) => {
    await services.Club.getStats().observe(
      cntlr.current(),
      async function _onGetStats(e, t) {
        e.unobserve(e);
        t.success
              ...
              if (e.type == "players") {
                if (e.count !== services.Club.clubDao.clubRepo.items.length) {
                  ...
                  for (let i = 0; i < playersPage; i++) {
                    ...
                    try {
                      const searchResult = await new Promise(
                        (resolve, reject) => {
                          services.Club.search(playersCriteria).observe(
                            e,
                            (p, t) => {
                              if (
                                (p.unobserve(p),
                      ...
                      services.Notification.queue([
                        services.Localization.localize(
                          "notification.club.failedToLoad",
                        ),
                  ...
                  if (
                    ...
                    !cntlr
                      .current()
                      ._view._objectivesTile.__root.querySelector(
                        ".fsu-task",
                      ) &&
                    cntlr
                      .current()
                      ._view._objectivesTile.__tileContent.querySelector(
                        ".ut-tile-view--subtitle",
                      )
                  ) {
                    cntlr
                      .current()
                      ._view._objectivesTile.__tileContent.before(
                        events.createDF(
                          `<div class="fsu-task">${info.task.obj.html}</div>`,
                        ),
                      );
                  }
                  if (
                    ...
                    !cntlr
                      .current()
                      ._view._sbcTile.__root.querySelector(".fsu-task") &&
                    cntlr
                      .current()
                      ._view._sbcTile.__tileContent.querySelector(
                        ".ut-tile-content-graphic-info",
                      )
                  ) {
                    cntlr
                      .current()
                      ._view._sbcTile.__tileContent.before(
                        events.createDF(
                          `<div class="fsu-task">${info.task.sbc.html}</div>`,
                        ),
                      );
                  }
                }
  ...
  UTHomeHubView.prototype.getObjectivesTile = function () {
    if (
      info.task.obj.html &&
      !this._objectivesTile.__root.querySelector(".fsu-task") &&
      info.set.info_obj
    ) {
  ...
  UTObjectivesHubTileView.prototype.setNumUnclaimedObjectives = function (e) {
    ...
    let currentNumber =
      this.getContentView().__unclaimedRewardsBubble.textContent == ""
        ? 0
        : Number(this.getContentView().__unclaimedRewardsBubble.textContent);
    let rewardsNumber =
      services.Objectives.objectivesRepository.getActiveCampaign()
        .remainingRewards
        ? services.Objectives.objectivesRepository.getActiveCampaign()
            .remainingRewards
        : 0;
  ...
  UTHomeHubView.prototype.getSBCTile = function () {
    if (
      info.set.info_sbc &&
      info.task.sbc.html &&
      !this._sbcTile.__root.querySelector(".fsu-task")
    ) {
  ...
  call.search = {
    club: {
      ...
      modeChange:
        UTClubSearchFiltersViewController.prototype.onSearchModeChanged,
  ...
  //改变为假想球员后禁止评分部分调整
  UTClubSearchFiltersViewController.prototype.onSearchModeChanged = function (
    t,
    e,
  ) {
    ...
    if (this.squad.isSBC) {
      ...
      if ("_fsuRatBox" in view) {
        view._fsuRatBox.querySelector("h4").textContent = fy(
          state ? "sbcrange.title" : "sbcrange.concepttitle",
        );
        ...
        if (!state) {
          ...
          view._fsuRatLow._incrementBtn.setInteractionState(state);
          view._fsuRatHigh._decrementBtn.setInteractionState(state);
          view._fsuRatHigh._incrementBtn.setInteractionState(state);
        }
      }
    }
  };
  ...
  //列表化学差异处给在俱乐部的球员上tag
  UTClubSearchResultsView.prototype.setItemsWithChemDiff = function (
    t,
    a,
    s,
    l,
    ...
    _.map(t, (player, index) => {
      if (
        player.concept &&
        !this.activeSquad.containsItem(player, !0) &&
        events.getItemBy(1, { definitionId: player.definitionId }).length
      ) {
        console.log(player);
        ...
        tag
          ...
          .classList.add("fsu-inclubtag");
        this._list
          .getRows()
          [index].__rowContent.appendChild(tag.getRootElement());
  ...
  UTItemSearchView.prototype.setFilters = function (e, t) {
    ...
    //选项球员数量统计
    if (
      e.searchCriteria.type == "player" &&
      e.searchFeature == "club" &&
      info.set.sbc_icount
    ) {
  ...
  events.clubSearchSetCount = (e) => {
    if (e.hasOwnProperty("_inputFill")) {
      ...
      if (view && view.className !== "UTSquadBuilderViewController") {
        if (view?.squad?.isSBC() || view?._squad?.isSBC()) {
          let fsuRatView =
            "_fsuRatLow" in view
              ? view
              : "_fsuRatLow" in view.getView()
                ? view.getView()
                : "_fsuRatLow" in view.getParentViewController()
                  ? view.getParentViewController()
                  : null;
          if (fsuRatView) {
            if ("_fsuRatLow" in fsuRatView && "_fsuRatHigh" in fsuRatView) {
              s["BTWrating"] = [
                fsuRatView._fsuRatLow.getValue(),
                fsuRatView._fsuRatHigh.getValue(),
              ];
            }
          }
          let fsuFillView =
            "_fsuFillArray" in view
              ? view
              : "_fsuFillArray" in view.getView()
                ? view.getView()
                : "_fsuFillArray" in view.getParentViewController()
                  ? view.getParentViewController()
                  : null;
          if (fsuFillView) {
      ...
      let l = isPhone()
        ? e.__picker.querySelectorAll("option")
        : e.__list.querySelectorAll("li");
      for (let [i, v] of l.entries()) {
        let id = e.options[i].id,
          ct = cm.get(id);
        if (ct) {
          if (isPhone()) {
  ...
  events.searchFill = async (e) => {
    let c = e.viewmodel.searchCriteria,
      t = e._parentViewController._fsuFillType,
      p = e._parentViewController._fsuFillArray,
      fs = e._parentViewController._fsuFillSort || 3,
      r =
        "_fsuFillRange" in e._parentViewController
          ? e._parentViewController._fsuFillRange
          : [46, 99];

    if (
      e.squad.isSBC() &&
      "_fsuRatLow" in e._view == false &&
      "_fsuRatHigh" in e._view == false &&
      t % 2 !== 1
    ) {
    ...
    if (t !== 1 && t % 2 == 1) {
      let s = new UTSearchCriteriaDTO(),
        not,
        sort = _.split(
          _.replace(_.toLower(SearchSortID[fs]), "rating", "ovr"),
          "_",
        );
      s._type = "player";
      s.count = 21;
      switch (t) {
        case 3:
          s.sortBy = Object.keys(info.criteria).length
            ? info.criteria.sortBy
            : sort[0];
          s._sort = Object.keys(info.criteria).length
            ? info.criteria._sort
            : sort[1];
    ...
    if (t && t % 2 == 0) {
      ...
      await e._view
        .getPlayerNameSearch()
        ._playerNameInput.setInteractionState(0);
      let sortId =
        SearchSortID[
          _.toUpper(`${_.replace(c.sortBy, "ovr", "rating")}_${c.sort}`)
        ];
    ...
    if (Object.keys(info.criteria).length == 0 && t == 0) {
      ...
      if (info.criteria._untradeables == "true") {
        await e._view
          .getSortOptions()
          .toggles._collection["sort-untradeable"].toggle(true);
      } else {
        await e._view
          .getSortOptions()
          .toggles._collection["sort-untradeable"].toggle(false);
      }
      //排除队伍选项匹配判断
      if (cntlr.current().className == `UTSquadSplitViewController`) {
        ...
        if (info.criteria.excludeDefIds.length > 0) {
          await e._view
            .getSortOptions()
            .toggles._collection["sort-exclude-squad"].toggle(true);
        } else {
          await e._view
            .getSortOptions()
            .toggles._collection["sort-exclude-squad"].toggle(false);
        }
      }
      //排序条件选项匹配判断
      if (
        info.criteria.sortBy !== c.sortBy ||
        info.criteria._sort !== c._sort
      ) {
      ...
      if (info.criteria.rarities !== c.rarities) {
        if (info.criteria.rarities.length == 1) {
          await e._view._filterContainer.filters[1].setIndexById(
            info.criteria.rarities[0],
          );
        }
      }
      //位置条件选项匹配判断
      if (info.criteria._position == "any") {
        ...
        let posId = -1;
        let slot = isPhone()
          ? cntlr.current().getCurrentController().iterator
          : cntlr.right().iterator;
        if (slot) {
      ...
      if (info.criteria.nation !== c.nation) {
        await e._view._filterContainer.filters[4].setIndexById(
          info.criteria.nation,
        );
      }
      if (info.criteria.league !== c.league) {
        await e._view._filterContainer.filters[5].setIndexById(
          info.criteria.league,
        );
      }
      if (info.criteria.club !== c.club) {
        await e._view._filterContainer.filters[6].setIndexById(
          info.criteria.club,
        );
      }
      if (t == 1) {
  ...
  events.searchInput = (c) => {
    ...
    for (let i of ["club", "nation", "league"]) {
      ...
      if (
        st == services.Localization.localize(`sbc.requirements.subType.${i}`)
      ) {
    ...
    if (e.type == "input") {
      setTimeout(() => {
        if (info.base.input) {
          ...
          let z = iz.hasOwnProperty("_filterContainer")
            ? iz._filterContainer.searchFilters._collection[f]
            : iz._searchFilters.searchFilters._collection[f];
          ...
          e.target._oData.forEach(function (el, i) {
            let a = document.querySelectorAll(p)[i],
              c =
                info.set.sbc_icount && "_oCount" in e.target
                  ? e.target._oCount[i] >= Number(v)
                    ? true
                    : false
                  : false;
            if (el.includes(v) || c) {
    ...
    for (const key of sectionKey) {
      ...
      if (list.length) {
        ...
        if (solePlayers.length && info.set.player_transfertoclub) {
          ...
          controller._header
            .getRootElement()
            .appendChild(controller._fsuSendClub.getRootElement());
        }
      }
    }
  };
  ...
  UTUnassignedItemsViewController.prototype.renderView = function (...args) {
    ...
    //生成头部
    let UTSPrice = 0;
    let UTSPlayer = this.getViewModel()
      .getSectionItems()
      .map((i) => {
        if (!i.isLoaned() && i.isPlayer()) {
    ...
    if (UTSPlayer.length) {
      ...
      UTSText.textContent = fy([
        "screenshot.text",
        UTSPlayer.length,
        UTSPrice.toLocaleString(),
      ]);
      this._view.__root.insertBefore(UTSSection, this._view.__root.firstChild);
    }
    //快捷按钮调出
    //可交换页面
    let itemSection = this.getView().getSection(
      UTUnassignedItemsViewModel.SECTION.ITEMS,
    );
    if (itemSection) {
      let items = this.getViewModel().getStorableSection();
      let vacancy =
        repositories.Item.numItemsInCache(ItemPile.TRANSFER) + items.length <=
        repositories.Item.getPileSize(ItemPile.TRANSFER);
      if (
        0 < items.length &&
        vacancy &&
        this.getViewModel().hasTradeableItems()
      ) {
        ...
        itemSection._header
          .getRootElement()
          .appendChild(this._view._fsuTransfer.getRootElement());
      }
      if (this.getViewModel().hasMovableItems()) {
        ...
        itemSection._header
          .getRootElement()
          .appendChild(this._view._fsuClub.getRootElement());
      }
    }

    let duplicateSection = this.getView().getSection(
    ...
    if (duplicateSection) {
      if (this.getViewModel().hasTradeableDuplicates()) {
        ...
        if (0 < items.length && vacancy) {
          ...
          duplicateSection._header
            .getRootElement()
            .appendChild(this._view._fsuDupTransfer.getRootElement());
        }
      }
      let duplicatePlayerId = duplicateSection.listRows.map((i) => {
        return i.data.duplicateId;
      });
      let duplicatePlayer = events
        .getItemBy(2, { id: duplicatePlayerId })
        .filter((i) => !i.untradeable);
      if (duplicatePlayer.length) {
        ...
        duplicateSection._header
          .getRootElement()
          .appendChild(this._view._fsuSwap.getRootElement());
      }
    }
    ...
    if (duplicateSection || itemSection) {
      ...
      let targetElement = (
        itemSection || duplicateSection
      )?._header?.getRootElement();
      if (targetElement) {
    ...
    let invalidPick = _.filter(
      this.getViewModel().getSectionItems(),
      (item) => {
    ...
    if (invalidPick.length) {
      _.map(this.getView().sections, (section) => {
        _.map(section.listRows, (item) => {
          if (
            item.data.isPlayerPickItem() &&
            item.data.id === item.data.definitionId
          ) {
            item.hide();
          }
        });
    ...
    setTimeout(() => {
      if (
        this.getViewModel() &&
        this.getViewModel().length === 0 &&
        !document.querySelector(".player-picks-modal")
      ) {
        if (isPhone()) {
          this._parentViewController._backButton._tapDetected(this);
        } else {
          this._parentViewController._parentViewController._backButton._tapDetected(
            this,
          );
        }
  ...
  UTSquadBuilderViewModel.prototype.generatePlayerCollection = function (
    ...
    let p = o.filter(
      (item) => !ls.includes(item.leagueId) && !rs.includes(item.rareflag),
    );
    let v = 0;
    for (let i = 0; i < 11; i++) {
    ...
    if (!this._generated) {
      ...
      if (info.base.promo && info.set.info_pack && "n" in info.base.promo) {
        let f = events.createButton(
          ...
          (e) => {
            let openUrl =
              "u" in info.base.promo
                ? info.base.promo.u
                : "https://www.futbin.com/home-tab/latest-promo";
        ...
        this.__root
          .querySelector(".ut-store-pack-details-view--pack-counts")
          .append(this._fsuPromoBox);
      }
    }
  };
  ...
  fsuSV.prototype._generate = function _generate() {
    if (!this._generated) {
      ...
      (info.set.card_style == 1 ? this._cStyle.old : this._cStyle.new).toggle(
        1,
      );
      ...
      this._fsuinfo = events.createButton(
        ...
        () => {
          GM_openInTab(
            `https://mfrasi851i.feishu.cn/wiki/OLNswCYQciVKw8k9iaAcmOY1nmf`,
            { active: true, insert: true, setParent: true },
          );
        },
        "call-to-action",
  ...
  events.getAuction = async function (e) {
    e.setInteractionState(0);
    e.__root.querySelector("span.btn-subtext").textContent = fy(
      "quicklist.getpriceload",
    );
    ...
    if (Object.keys(p_cm).length) {
      pdb[id] = Object.entries(p_cm)[0][0].toLocaleString("en-US", {
        style: "decimal",
      });
      let p_e = document.querySelector(`button.fsuGP[data-id='${id}']`);
      if (p_e) {
        p_e.querySelector("span.btn-subtext").textContent =
          pdb[id].toLocaleString();
  ...
  events.buyPlayer = async (player, view) => {
    ...
    if (
      repositories.Item.numItemsInCache(ItemPile.PURCHASED) >= MAX_NEW_ITEMS
    ) {
      events.notice(
        ["buyplayer.error", playerName, fy("buyplayer.error.child5")],
        2,
      );
      state = false;
    } else {
      ...
      if (!priceList || priceList.length == 0) {
        events.notice(
          ["buyplayer.error", playerName, fy("buyplayer.error.child3")],
          2,
        );
        state = false;
      } else {
        ...
        if (
          !currentData.canBuy(
            services.User.getUser().getCurrency(GameCurrency.COINS).amount,
          )
        ) {
          events.notice(
            ["buyplayer.error", playerName, fy("buyplayer.error.child2")],
            2,
          );
          state = false;
        } else {
          if (0 < currentData.getSecondsRemaining()) {
            return new Promise(async (resolve) => {
              ...
              services.Item.bid(
                currentPlayer,
                currentPlayer._auction.buyNowPrice,
                ...
                if (data.success) {
                  ...
                  services.Item.move(currentPlayer, ItemPile.CLUB).observe(
                    this,
                    (e, t) => {
                      if ((e.unobserve(this), t.success)) {
                        events.notice(
                          ["buyplayer.sendclub.success", playerName],
                          0,
                        );
                        if (isPhone()) {
                          let controller = cntlr.current();
                        ...
                        events.notice(
                          ["buyplayer.sendclub.error", playerName],
                          2,
                        );
                        state = false;
                      }
                  ...
                  let denied =
                    data.error &&
                    data.error.code === UtasErrorCode.PERMISSION_DENIED;
                  events.notice(
                    [
                      "buyplayer.error",
                      playerName,
                  ...
                  if (view) {
                    view
                      .getSuperview()
                      .items._collection[
                        view.getSuperview().items._index
                      ].render(player);
                  }
                  events.hideLoader();
                }
            ...
            events.notice(
              ["buyplayer.error", playerName, fy("buyplayer.error.child4")],
              2,
            );
            state = false;
          }
  ...
  events.cardAddBuyErrorTips = (defId) => {
    ...
    if (!isPhone()) {
      _.map(squad._fsuBuyEroor, (i) => {
        if (
          document.querySelector(`.fsu-cards-buyerror[data-id="${i}"]`) ==
            null &&
          document.querySelector(`.fsu-cards-price[data-id="${i}"]`) !== null
        ) {
          let buyErrorElement = events.getBuyErrorTipsHtml();
          let targetElement = document.querySelector(
            `.ut-squad-slot-view .concept .fsu-cards-price[data-id="${i}"]`,
          ).parentNode;
  ...
  events.getBuyErrorTipsHtml = () => {
    let buyError = events.createElementWithConfig("div", {
      classList: [
        "ut-squad-slot-chemistry-points-view",
        "item",
        "fsu-cards",
        "fsu-cards-buyerror",
      ],
    ...
    let buyErrorIcon = events.createElementWithConfig("div", {
      classList: [
        "ut-squad-slot-chemistry-points-view--container",
        "chemstyle",
        "icon_untradeable",
      ],
    });
  ...
  events.readAuctionPrices = async (player, price) => {
    ...
    if (searchCriteria.defId.length) {
      ...
      if (price) {
        ...
        let playerPrice = await getFutbin(
          `https://www.futbin.com/${info.base.year}/playerPrices?player=${defId}`,
        );
        if (!playerPrice) {
          return;
        }
      ...
      while (attempts-- > 0) {
        events.changeLoadingText([
          "readauction.loadingclose3",
          `${searchModel.searchCriteria.maxBuy.toLocaleString()}`,
        ]);
        if (queried.includes(searchModel.searchCriteria.maxBuy)) {
          break;
        }
        services.Item.clearTransferMarketCache();
        let response = await events.searchTransferMarket(
        ...
        if (response.success) {
          ...
          if (response.data.items.length == 0) {
            currentQuery =
              UTCurrencyInputControl.getIncrementAboveVal(currentQuery);
          } else if (response.data.items.length == 21) {
            currentQuery =
              UTCurrencyInputControl.getIncrementBelowVal(currentQuery);
          } else {
            break;
          }
  ...
  events.searchTransferMarket = (criteria, type) => {
    return new Promise(async (resolve) => {
      services.Item.searchTransferMarket(criteria, type).observe(
        this,
  ...
  UTSBCSquadDetailPanelView.prototype.render = function (e, t, i, o) {
              ...
              (t, i) => {
                if (t === 2) {
                  ...
                  if (v == "") {
                    ...
                    let futBinRegex =
                      /www.futbin.com\/\d{2}\/squad\/\d{9}|^\d{9}$/;
                    let futGGRegex =
                      /www.fut.gg\/\d{2}\/squad-builder\/\S{8}-\S{4}-\S{4}-\S{4}-\S{12}|^\S{8}-\S{4}-\S{4}-\S{4}-\S{12}$/;
    ...
    //计算所需条件
    let sc = 11 - e.squad.getAllBrickIndices().length,
      gf = [],
      gfall = {},
      targetRting = 0,
      needChem = false;
    for (let i of e.eligibilityRequirements) {
      let fk = i.getFirstKey(),
      ...
      if (
        fk == SBCEligibilityKey.PLAYER_MIN_OVR &&
    ...
    if (!this._fsuIgnore) {
      ...
      this._challengeDetails._requirements.__root.appendChild(
        this._fsuIgnore.__root,
      );
      ...
      if (!this._fsuAutoFill && info.set.sbc_autofill) {
        let b = events.createButton(
          ...
          (e) => {
            ...
            if (
              !info.build.strictlypcik &&
              _.size(e._gf) == 2 &&
              _.isEqual(_.sortBy(_.keys(e._gf[0].t)), ["gs", "rs"]) &&
              _.isEqual(_.sortBy(_.keys(e._gf[1].t)), ["gs", "rs"]) &&
              e._gf[0].t.rs == e._gf[1].t.rs
            ) {
              ...
              pl = events
                .getItemBy(
                  2,
                  gf,
                  services.Item.itemDao.itemRepo.getUnassignedItems(),
                )
                .slice(0, criteriaNumber);
            } else {
              for (let i of e._gf) {
        ...
        this._fsuAutoFill = b;
        this._challengeDetails._requirements.__root.appendChild(
          this._fsuAutoFill.__root,
        );
        this._fsuIgnore.show();
      }
    } else if (
      !this._fsuDupFill &&
      info.set.sbc_dupfill &&
      repositories.Item.getUnassignedItems().length
    ) {
      let b = events.createButton(
        new UTStandardButtonControl(),
        fy("dupfill.btntext"),
        (e) => {
          let gf = {
            id: [
              ...new Set(
                repositories.Item.getUnassignedItems().map((i) => {
                  if (i.isDuplicate() && !i.isLoaned() && i.isPlayer()) {
    ...
    if (
      info.set.sbc_dupfill &&
      this._fsuDupFill &&
      !services.Item.itemDao.itemRepo.getUnassignedItems().length
    ) {
    ...
    if (!this._fsuSquadCmpl && info.set.sbc_squadcmpl && targetRting) {
      let b = events.createButton(
        ...
        (e) => {
          ...
          let va =
              e._parent.squad.getNumOfRequiredPlayers() -
              e._parent.squad.getFieldPlayers().filter((i) => i.isValid())
                .length,
            fillRating = events.needRatingsCount(e._parent.squad, e._target),
            inputText =
              fillRating[0].length && e._target
                ? [
                    fy("squadcmpl.placeholder"),
                    fillRating[0].length == "0" && va == 0
                      ? ""
                      : fillRating[0].join(`,`),
                  ]
                : fy(
                    va ? "squadcmpl.placeholder" : "squadcmpl.placeholder_zero",
                  ),
            popupBtns =
              e._target && info.set.sbc_top
                ? [
                    { labelEnum: enums.UIDialogOptions.OK },
                    { labelEnum: 44401 },
                    { labelEnum: enums.UIDialogOptions.CANCEL },
          ...
          events.popup(
            ...
            (t, i) => {
              if (t === 2) {
                ...
                if (reg.test(v)) {
                  a = v.split(",");
                  let p = events.getRatingPlayers(
                    e._parent.squad,
                    e._parent.eligibilityRequirements,
                    a,
                  );
              ...
              if (t === 44401) {
                let btn = isPhone()
                  ? cntlr.current().getView()._fsuCount
                  : cntlr.left().getView()._fsuCount;
                btn._tapDetected();
              }
            },
            popupBtns,
    ...
    if (needChem) {
      ...
      if (
        "_fsuSquadCmpl" in this &&
        this.getRootElement().querySelector(".fsu-onlycmpltext") == null
      ) {
        ...
        this._fsuSquadCmpl
          ...
          .parentNode.insertBefore(
            only,
            this._fsuSquadCmpl.getRootElement().nextSibling,
          );
      }
    }
  };
  ...
  events.playerListFillSquad = (challenge, list, type) => {
    ...
    let squadBestPos = squadFormation.generalPositions.concat(
      Array(12).fill(-1),
    );
  ...
  events.getTemplate = async function (e, type, sId) {
    ...
    for (let planId of refePlan) {
      ...
      let copySquadPos = JSON.parse(
        JSON.stringify(e.challenge.squad.getFormation().generalPositions),
      );
      for (let i = 0; i < createSquad.length; i++) {
        ...
        if (type == 3) {
          if (
            ...
            player.definitionId =
              planSquad.data.activeGroupPositions[i].playerEaId;
            player.stackCount = 1;
            let cachePlayer = events.getItemBy(2, {
          ...
          if (squadPos[posIndex] !== null) {
            if (planIndex in planSquad) {
              ...
              player.stackCount = 1;
              let cachePlayer = events.getItemBy(2, {
                definitionId: planPlayer.Player_Resource,
              })[0];
              if (cachePlayer) {
                player.id = cachePlayer.id;
                player.concept = false;
    ...
    await events.saveSquad(
      ...
      resultSquad
        .map((i) => {
          if (i && !info.roster.data.hasOwnProperty(i.definitionId)) {
            return i.definitionId;
          }
        })
        .filter(Boolean),
  ...
  function getSbcSquad(i, t) {
    ...
    let u =
      t == 1
        ? `https://www.futbin.org/futbin/api/getChallengeTopSquads?chal_id=${i}&platform=${p}`
        : t == 2
          ? `https://www.futbin.org/futbin/api/getSquadByID?squadId=${i}`
          : `https://www.fut.gg/api/squads/${i}`;
    return new Promise((res) => {
      GM_xmlhttpRequest({
        method: "GET",
        url: u,
  ...
  events.playerToAuction = async (d, p) => {
    let i =
      services.Item.itemDao.itemRepo.transfer.get(d) ||
      services.Item.itemDao.itemRepo.unassigned.get(d) ||
      services.Item.itemDao.itemRepo.club.items.get(d);
    let t =
      services.Item.itemDao.itemRepo.transfer._collection.hasOwnProperty(d);
    if (i) {
      if (
        (repositories.Item.getPileSize(ItemPile.TRANSFER) -
          repositories.Item.numItemsInCache(ItemPile.TRANSFER) >
        ...
        if (i.hasPriceLimits()) {
          if (
            p < i._itemPriceLimits.minimum ||
            p > i._itemPriceLimits.maximum
          ) {
        ...
        await services.Item.list(i, lp, p, 3600).observe(
          cntlr.current(),
          async (e, t) => {
            if ((e.unobserve(cntlr.current()), t.success)) {
              events.notice(
                ["notice.auctionsuccess", i._staticData.name, p],
                0,
              );
            } else {
  ...
  call.squad = {
    ...
    requirements:
      UTSBCChallengeRequirementsView.prototype.renderChallengeRequirements,
  };
    ...
    function (n, r) {
      ...
      setTimeout(() => {
        if (cntlr.current().className.includes("UTSBCSquad")) {
          let view = this;
          let ratingEligibility = n.eligibilityRequirements.filter(
            (i) => i.getFirstKey() == 19,
          );
          let requests = [];
  ...
  events.saveOldSquad = (s, t, c) => {
    if (s.isSBC() && (!info.base.savesquad || !t)) {
      ...
      if (
        ...
        if (
          isPhone() &&
          cntlr.current().className == "UTSquadItemDetailsNavigationController"
        ) {
          setTimeout(() => {
            cntlr.current()._parentViewController._eBackButtonTapped();
          }, 500);
  ...
  events.getRatingPlayers = (squad, eligibility, ratings) => {
    let playerlist = [],
      ratingsList = ratings ? Array.from(ratings) : [];
    let Exclusionlist = squad
      .getPlayers()
      .map((i) => {
        if (i.item.rating && !i.item.concept) {
          return i.item.databaseId;
        }
      })
      .filter(Boolean);
      ...
      if (i && !i.isBrick()) {
        if (i.isValid()) {
          ...
          if (player.concept) {
            ...
            let shortlist = events.getItemBy(
              2,
              need,
            ...
            if (shortlist.length) {
              if (eligibilityChemistry) {
                for (let p of shortlist) {
                  let fieldPlayers = squad.getFieldPlayers().map(function (t) {
                    return t.item;
                  });
                  ...
                  fieldPlayers[t] = p;
                  if (
                    squad.chemCalculator.calculate(
                      formation,
                      fieldPlayers,
                      manager,
                    ).chemistry >= eligibilityChemistry
                  ) {
  ...
  UTUnassignedTileView.prototype.setNumberOfItems = function (e) {
    ...
    let item = _.filter(repositories.Item.getUnassignedItems(), (item) => {
      ...
      return (
        (item.isPlayer() && repeat.length === 0) ||
        (!item.isPlayer() && !item.isDuplicate() && !item.isMiscItem())
      );
    });
              ...
              (a, b) => {
                if ((a.unobserve(cntlr.current()), b.success)) {
                  ...
                  if (cntlr.current().className == "UTStoreHubViewController") {
                    ...
                    cntlr.current().className == "UTStorePackViewController"
                  ) {
                    if (repositories.Item.getUnassignedItems().length) {
                      e._parent.setNumberOfItems(
                        repositories.Item.getUnassignedItems().length,
                      );
                      e.hide();
                    } else {
            ...
            services.Item.requestUnassignedItems().observe(
              ...
              (p, t) => {
                ...
                if (t.success) {
                  ...
                  console.log(
                    _.filter(t.response.items, (i) =>
                      _.includes(defIds, i.definitionId),
                    ),
                  );
                  setUnassignedToClub(
                    _.filter(t.response.items, (i) =>
    ...
    setTimeout(() => {
      ...
      if (HideAndShow) {
        this.storePacks.forEach((item) => {
          if (
            info.base.packcoin.hasOwnProperty(item.articleId) &&
            !item.getRootElement().querySelector(".fsu-packprice")
          ) {
            let packCoinBox = document.createElement("p");
            packCoinBox.classList.add(
            ...
            item.__articleDesc.parentNode.insertBefore(
              packCoinBox,
              item.__articleDesc,
            );
          }
          ...
          for (const i of e) {
            if (i.id in filterOptionCount) {
              ...
              filterOptionCount[i.id] = {
                ...
                value: info.base.packcoin.hasOwnProperty(i.id)
                  ? info.base.packcoin[i.id]
                  : 0,
              };
            }
          }
      ...
      if (packFilter) {
        if (this.getStoreCategory() == "mypacks" && e.length) {
          ...
          let filterStart = 2;
          let tradeableCount =
            this.__itemList.querySelectorAll(".is-tradeable").length;
          let packTotal = _.sumBy(
            _.values(this._fsufilterOptionCount),
            "count",
          );
          let packValue = _.sum(
          ...
          filterOptionArray.push(
            new UTDataProviderEntryDTO(-1, -1, fy(`sbc.filter0`)),
          );
          filterOptionArray.push(
            new UTDataProviderEntryDTO(
              0,
              0,
              fy([`packfilter.total`, packTotal, packValue.toLocaleString()]),
            ),
          );
          if (tradeableCount) {
            filterOptionArray.push(
              new UTDataProviderEntryDTO(
                1,
                1,
                `${fy(`pack.filter0`)}×${tradeableCount}`,
              ),
            );
            filterStart = 3;
          }
          for (const key in this._fsufilterOptionCount) {
            ...
            let dto = new UTDataProviderEntryDTO(
              Number(key),
              Number(key),
          ...
          filterOption.addTarget(
            ...
            (e) => {
              ...
              e._parent.storePacks.forEach((i) => {
                if (
                  i.articleId == filterId ||
                  filterId == -1 ||
                  (filterId == 1 &&
                    i.getRootElement().classList.contains("is-tradeable"))
                ) {
                  i.show();
                } else {
                  i.hide();
                ...
                if (filterId == -1) {
                  e._parent.__itemList.addEventListener(
                    EventType.SCROLL,
                    e._parent.debounceCallback,
                    !1,
                  );
                } else {
                  e._parent.__itemList.removeEventListener(
                    EventType.SCROLL,
                    e._parent.debounceCallback,
                    !1,
                  );
      ...
      if (packTileExists || SBCTileExists) {
        ...
        tileBox.style.cssText =
          "display: flex;background: none; border: none; justify-content: space-between;";
        let tileStyle =
            info.set.info_packagain && info.set.info_sbcagain
              ? `margin:0;`
              : `margin:0;flex-basis: 100%;max-width: 100%;`,
          tileClass =
            info.set.info_packagain && info.set.info_sbcagain
              ? "col-1-2"
              : "col-1-1";
          ...
          let sbcTile = events.createTile(
            ...
            (e) => {
              if (info.douagain.sbc) {
                events.goToSBC(
                  services.SBC.repository.getSetById(info.douagain.sbc),
                );
              } else {
  ...
  events.needRatingsCount = (squad, target) => {
    ...
    let haveRatingsOriginal = _.map(events.getItemBy(2, criteria), "rating"),
      ...
      lackSimulation = Array.from({ length: haveRatings.length }, (_e, i) =>
    ...
    let simulated = _.multicombinations(
        _.slice(
          haveRatings,
          _.indexOf(haveRatings, basisRating) - fillOffset,
          _.indexOf(haveRatings, basisRating) + fillNumber - fillOffset,
        ),
        lackNumber,
      ),
      totalValue = 0,
    ...
    _.flatMap(simulated, (i) => {
      ...
      if (simulatedCount >= target) {
        ...
        if (
          (totalValue == 0 ||
            simulatedTotalValue < totalValue ||
            (simulatedTotalValue == totalValue &&
              simulatedSumRating < sumRating)) &&
          simulatedSatisfy
        ) {
          result = i;
  ...
  events.SBCListInsertToFront = (sbcId, type) => {
    ...
    if (
      ...
      info.douagain.SBCList = _.filter(
        info.douagain.SBCList,
        (SBCId) => !services.SBC.repository.getSetById(SBCId).isComplete(),
      );
      ...
      _.map(info.douagain.SBCList, (item, index) => {
        let button = events.createButton(
          ...
          (e) => {
            events.goToSBC(
              services.SBC.repository.getSetById(
                Number(e.__root.getAttribute("data-SBCId")),
              ),
            );
          },
          "",
        ...
        img.setAttribute(
          "src",
          AssetLocationUtils.getSquadBuildingSetImageUri(
            services.SBC.repository.getSetById(item).assetId,
          ),
        );
  ...
  events.doDailySBC = (SBCs) => {
    ...
    for (let i = 0; i < sbcSetEntities.length; i++) {
      events.autoDoSBC(sbcSetEntities[i]);
    }
  };

  events.autoDoSBC = async (SBCSetEntity) => {
    ...
    //  ].includes(cntlr.current().constructor)
    // ) {}
    console.log("wait for SBC squad loaded");
    events.showLoader();
    await events.wait(3, 4);
    let controller = cntlr.current(),
  ...
  events.goToSBC = (SBCSetEntity) => {
    ...
    services.SBC.requestChallengesForSet(SBCSetEntity).observe(
      controller,
      (e, t) => {
        if (
          (e.unobserve(controller), t.success && 0 < t.data.challenges.length)
        ) {
            ...
            (r = t.data.challenges[0]),
              services.SBC.loadChallenge(r).observe(controller, (ee, tt) => {
                if ((ee.unobserve(controller), tt.success)) {
                  ...
                  let n =
                    ee.error &&
                    tt.error.code === UtasErrorCode.SERVICE_IS_DISABLED
          ...
          let n =
            t.error && t.error.code === UtasErrorCode.SERVICE_IS_DISABLED
              ? "sbc.notification.disabled"
              : "notification.sbcChallenges.failedToLoad";
  ...
  events.setPackTileText = (packTile) => {
    if (!info.douagain.pack) {
      ...
      let pack = services.Store.storeDao.storeRepo.myPacks
        .values()
        .filter((i) => i.id == info.douagain.pack);
      if (pack.length) {
        packTile.setInteractionState(1);
        packTile.setDescription(
          `${services.Localization.localize(pack[0].packName)} (${pack.length})`,
        );
      } else {
        packTile.setInteractionState(0);
        packTile.setDescription(fy("douagain.packtile.text"));
  ...
  UTPackAnimationViewController.prototype.runAnimation = function () {
    if (!this.running) {
      ...
      this.animationTimeout = window.setTimeout(
        ...
        info.set.info_skipanimation ? 0 : 4500,
      );
    }
  };
  //开包设置再次开包
  UTStoreViewController.prototype.eOpenPack = function (p, e, t) {
    ...
    let i,
      d =
        null === (i = this.viewmodel) || void 0 === i
          ? void 0
          : i.getPackById(
              t.articleId,
              e === UTStorePackDetailsView.Event.OPEN,
              JSUtils.isBoolean(t.tradable) ? t.tradable : void 0,
            );
    if (d.isMyPack) {
      if (
        services.Store.storeDao.storeRepo.myPacks
          .values()
          .filter((i) => i.id == d.id).length > 1
      ) {
        ...
        if (
          !services.Store.storeDao.storeRepo.myPacks
            .values()
            .filter((i) => i.id == info.douagain.pack).length
        ) {
  ...
  //获得包和SBC进行存储
  UTSBCSquadOverviewViewController.prototype._onChallengeSubmitted =
    function _onChallengeSubmitted(e, t) {
  ...
  UTSelectItemFromClubViewController.prototype.requestItems = function () {
    if (
      this.clubViewModel.canShowPage() &&
      !this.clubViewModel.shouldRequestItems()
    ) {
      ...
      if (this.squad.isSBC()) {
        let searchView =
            this?.getParentViewController()?.getPreviousController()._view,
          type = this.getParentViewController()._fsuFillType,
          players = _.clone(this.getParentViewController()._fsuFillArray),
          sort = _.split(
            _.replace(
              _.toLower(
                SearchSortID[this.getParentViewController()._fsuFillSort],
              ),
              "rating",
              "ovr",
            ),
        ...
        if (searchView && _.isArray(players)) {
          if ("_fsuRatLow" in searchView && "_fsuRatHigh" in searchView) {
            range = [
              searchView._fsuRatLow.getValue(),
              searchView._fsuRatHigh.getValue(),
            ];
          } else {
            range = info.range;
          }
          if (
            (range.join("") !== "4699" && type < 3) ||
            (type > 3 && type % 2 == 0)
          ) {
            method = false;
            ...
            if (type < 3) {
              players = events.getDedupPlayers(
                events.getItemBy(2, { BTWrating: range }),
                this.squad.getPlayers(),
              );
            } else if (
              "_fsuFilterBtn" in this.getParentViewController() &&
              "_type" in this.getParentViewController()._fsuFilterBtn &&
              !this.getParentViewController()._fsuFilterBtn_type
            ) {
              players = this.clubViewModel.getItemList();
            } else {
              players = players
                .map((i) => {
                  if (i.rating >= range[0] && i.rating <= range[1]) {
                    return i;
                  }
                })
                .filter(Boolean);
        ...
        //当未选择位置时出现匹配当前位置的选项
        if (
          this.searchCriteria.position == "any" &&
          this.getView()
            .pinnedItemView.getRootElement()
            .querySelector(".fsu-chemistryfilter") == null
        ) {
          ...
          let slotPos =
            slotIndex > 10
              ? 0
              : this.squad.getFormation().getPosition(slotIndex);
          let filterPlayers =
            slotIndex > 10
              ? originalPlayers
              : _.filter(originalPlayers, (obj) =>
                  _.includes(obj.possiblePositions, slotPos.typeId),
                );

          if (
            filterPlayers &&
            filterPlayers.length &&
            filterPlayers.length < originalPlayers.length
          ) {
            let buttonType = filterPlayers.length < originalPlayers.length;
            let buttonText = buttonType
              ? [`chemistrylist.setpos`, slotPos.typeName]
              : `chemistrylist.setall`;
            let button = events.createButton(
              new UTStandardButtonControl(),
              fy(buttonText),
              (e) => {
                ...
                if (e._type) {
                  e._type = false;
                  let filterPlayers = _.filter(
                    e._current.clubViewModel.getItemList(),
                    (obj) => _.includes(obj.possiblePositions, e._pos.typeId),
                  );
                  e._parent._currentController.clubViewModel.resetCollection(
                    filterPlayers,
                  );
  ...
  UTGameRewardsViewController.prototype.onButtonTapped = function (e, t, i) {
    ...
    if (
      this.hasPackReward &&
      cntlr.current().className == "UTStorePackViewController" &&
      cntlr.current().getView().getStoreCategory() == "mypacks"
    ) {
  ...
  UTCampaignRewardsCarouselView.prototype.setupRewards = function (e) {
    ...
    if (e.length) {
      const rewardElement = this?.__root?.querySelectorAll(
        ".objective-reward-label",
      );
      if (rewardElement && rewardElement.length == e.length) {
        for (let i = 0; i < e.length; i++) {
          let reward = e[i],
            packCoinValue = info.base.packcoin?.[reward.value];
      ...
      if (rewardElement && rewardElement.length == e.length) {
        for (let i = 0; i < e.length; i++) {
          if (e[i].rewards.length == 1) {
            let reward = e[i].rewards[0],
              packCoinValue = info.base.packcoin?.[e[i].rewards[0].value];
            if (reward.isPack && packCoinValue) {
              let packCoin = events.createElementWithConfig("span", {
                classList: ["currency-coins"],
  ...
  UTRewardSelectionChoiceViewController.prototype.viewDidAppear = function () {
    ...
    let reward = [];
    if (isSelected) {
      reward = Array.from(this.rewardSets).flatMap(
        (rewardSet) => rewardSet.awards,
      );
    } else {
      reward = this.rewardSets.map((set) => set.awards[0]);
    }
    let rewardElement = this.getView().__rewardTiles.querySelectorAll(
      isSelected ? ".textField" : ".main-description",
    );
    console.log(reward, rewardElement);
    if (reward.length == rewardElement.length) {
      for (let i = 0; i < rewardElement.length; i++) {
        let e = rewardElement[i],
          r = reward[i],
          packCoinValue = info.base.packcoin?.[r.value];
        if (
          !e.querySelector("span.currency-coins") &&
          r.isPack &&
          packCoinValue
        ) {
          let packCoin = events.createElementWithConfig("span", {
  ...
  UTRewardsCarouselView.prototype.setupRewards = function (t) {
    ...
    if (t.length) {
      const rootElement = this?.__root,
        packCoinValue = info.base.packcoin?.[t[0].value];
  ...
  UTClubHubView.prototype.clearTileContent = function (...args) {
    ...
    if ("_fsuLockTile" in this) {
      ...
      this.getRootElement()
        ...
        .appendChild(this._fsuLockTile.getRootElement());
    }
  };
        ...
        services.Club.search(t).observe(this, function (e, t) {
          ...
          if (
            ...
            services.Notification.queue([
              ...
              null === (i = s.getNavigationController()) ||
                void 0 === i ||
  ...
  //搜索球员时抓取所搜索的球员内容
  UTMarketSearchFiltersView.prototype.eSearchButtonSelected = function (
    e,
    t,
    i,
  ) {
    ...
    if (
      cntlr.current().className == "UTMarketSearchFiltersViewController" &&
      this.pinnedListRowItem == null
    ) {
      let criteria = JSON.parse(
        JSON.stringify(cntlr.current().viewmodel.searchCriteria),
      );
  ...
  UTMarketSearchFiltersView.prototype.setFilters = function (e, t) {
    ...
    if (e.searchBucket == 0 && e.showCategoryTab) {
      ...
      if (!("_fsuHistory" in this)) {
        ...
        this.getRootElement()
          .querySelector(".ut-pinned-list")
          .appendChild(this._fsuHistory.element);
      }
      ...
      if (this._fsuHistory.ts !== info.market.ts) {
        ...
        for (const i of info.market.mb) {
          let info = repositories.Item.getStaticDataByDefId(
            i[criteriaKeys.indexOf("maskedDefId")],
          );
          let btn = events.createButton(
            new UTStandardButtonControl(),
              ...
              if (!(keys.length - e.criteria.length)) {
                keys.forEach(function (value, index) {
                  ...
                  if (Array.isArray(current[value])) {
                    condition =
                      current[value].length !== e.criteria[index].length;
                  } else {
          ...
          let bid = [];
          if (
            i[criteriaKeys.indexOf("minBid")] +
              i[criteriaKeys.indexOf("maxBid")] >
            0
          ) {
            bid = [
              i[criteriaKeys.indexOf("minBid")],
            ...
            bid = [
              i[criteriaKeys.indexOf("minBuy")],
              i[criteriaKeys.indexOf("maxBuy")],
              "auctioninfo.buynowprice",
            ];
          }
          let defaultText = services.Localization.localize(
            "search.comboBoxDefaultValue",
          );
          ...
          this._fsuHistory.element
            .querySelector(".fsu-historybox")
            .appendChild(eblock);
        }
    ...
    if ("_fsuGoToFutbin" in this.getView()) {
      ...
      let id = this.viewmodel.getSelectedSlot().id,
        ...
        b = events.createButton(
          ...
          fy("quicklist.gotofutbin"),
          (e) => {
            GM_openInTab(`https://www.futbin.com/evolutions/${e.id}/`, {
              active: true,
              insert: true,
              setParent: true,
            });
      ...
      styleElement.textContent =
        '.fsu-academyFutbin::before { content:"\\E0DF" !important}';
    ...
    if (this.viewmodel !== void 0) {
      let searchCategoryIds = _.map(
        _.filter(
          ...
          (obj) =>
            obj.localizedName === "FUT_STORE_CAT_SPECIAL_NAME" ||
            obj.localizedName === "FUT_STORE_CAT_GLOBAL_L",
        ),
        "categoryId",
      );
      _.map(this.getView()._navigation.items, (item) => {
        if (searchCategoryIds.includes(item.id)) {
          let coinsPack = _.filter(
            this.viewmodel.getCategoryArticles(item.id),
            (pack) =>
              _.isEqual(pack.state, "active") &&
              _.size(pack.prices._collection) === 1 &&
  ...
  events.createVirtualChallenge = (c) => {
    ...
    if (c.squad.simpleBrickIndices.length) {
      ...
      for (let i = 0; i < 11; i++) {
        brickIndices.push({
          index: i,
          playerType: c.squad.simpleBrickIndices.includes(i)
            ? "BRICK"
            : "DEFAULT",
        });
      }
    ...
    let newSquad = new UTSquadEntity(
      ...
      new UTSquadChemCalculatorUtils(
        services.Chemistry,
  ...
  lock.init = function () {
    let a = JSON.parse(GM_getValue("lock", "[]")),
      b = [];
    if (a && typeof a === "object") {
      b = a;
    }
